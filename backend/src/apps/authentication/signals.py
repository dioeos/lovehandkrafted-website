...
def update_user_verification_status(sender, instance, **kwargs):
    ...
    if instance.verified:  # If email is marked as verified
        user = instance.user
        print(f"Before update: is_verified={user.is_verified}, is_vendor={user.is_vendor}")
        if user and not user.is_verified:
            user.is_verified = True
            user.save(update_fields=["is_verified"])
            print(f"After update: is_verified={user.is_verified}, is_vendor={user.is_vendor}")

#! not causing the vendor update issue
